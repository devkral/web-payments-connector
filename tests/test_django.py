...
from web_payments_dummy.django_dummy.models import QPayment
from web_payments.django.urls import _process_data
from web_payments import (
    ProviderVariant,
    PROVIDER_CACHE,
    PaymentStatus,
)
...
class TestHelpers(TestCase):
    ...
    def test_model(self):
        QPayment.load_providers()
        testp = QPayment()
        for i in QPayment.list_providers():
...
class TestUrl(TestCase):
    ...
    def test_body(self, save, success, failure):
        ...
        testp = QPayment(variant="DummyProvider")
        self.assertEqual(
            _process_data(request, testp, testp.provider).url,
            testp.get_success_url(),
        )
